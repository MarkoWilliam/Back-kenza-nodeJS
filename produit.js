...
exports.allnotif = (req, res) => {
        try {
            db.query("SELECT `id_notification`,`titre`,`contenu`,`etat`, DATE_FORMAT(date_add,'%d/%m/%Y %H:%i') as date_add,`id_product`,`id_attribute`,`id_category`,`nom`,`id_image`,`link_rewrite`,`id_product_attribute`,`image`  FROM notification", async(error, results) => {
                /* console.log(results);  */
                if (results) {
                    return res.json(results);
                } else {
